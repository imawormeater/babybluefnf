...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'nap-time' | 'kidz-bop':
			{
					curStage = 'crib';
					defaultCamZoom = 1;
					curStage = 'stage';
					var bg:FlxSprite = new FlxSprite(-200, -540).loadGraphic(Paths.image('baby/floor'));
					bg.antialiasing = true;
					bg.scrollFactor.set(1, 1);
					bg.active = false;
					add(bg);
			}
			case 'baby-blue'  | 'kadeisadipshit':
			{
					curStage = 'crib2';
					defaultCamZoom = 1;
					curStage = 'stage';
					var bg:FlxSprite = new FlxSprite(-200, -540).loadGraphic(Paths.image('baby/floor2'));
					bg.antialiasing = true;
					bg.scrollFactor.set(1, 1);
					bg.active = false;
					add(bg);
	...
	function generateRanking():String
	{
		...
		var wifeConditions:Array<Bool> = [
			accuracy >= 99.9935, // EPIC!!!
			accuracy >= 99.980, // EPIC!!:
			accuracy >= 99.970, // EPIC!.
			accuracy >= 99.955, // EPIC
			accuracy >= 99.90, // AAA:
			accuracy >= 99.80, // AAA.
			accuracy >= 99.70, // AAA
			accuracy >= 99, // AA:
